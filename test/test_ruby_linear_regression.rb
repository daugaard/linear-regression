...
class RubyLinearRegressionTest < Minitest::Test
  ...
  def test_can_predict_with_know_data_using_gradient_descent
    x_data = []
    y_data = []
    # Load data from CSV file into two arrays - one for independent variables X and one for the dependent variable Y
    CSV.foreach("data/ex1data1.txt", :headers => false) do |row|
    ...
    linear_regression.train_gradient_descent(0.01, 1000)
  ...
  def test_can_make_predicition_with_1_independent_variable_gradient_descent
    linear_regression = RubyLinearRegression.new

    linear_regression.load_training_data [[0],[10],[20]], [1,2,3]

    linear_regression.train_gradient_descent

    assert_equal linear_regression.predict([5]).round(1), 1.5
  end

  def test_can_make_predicition_with_2_independent_variable_gradient_descent
    linear_regression = RubyLinearRegression.new
  ...
  def test_can_make_predicition_with_4_independent_variable_gradient_descent
    linear_regression = RubyLinearRegression.new
